...
function generateDailyObjArr(id = 1, kidId = 1, userId = 1) {
  let dailyRecordArr = [];
...
function getMonthDate(str, year = 0, month = 0) {
  // 创建当前日期的 Date 对象
  const currentDate =
    (year == 0) & (month == 0) ? new Date() : new Date(year, month - 1, 1);
  // 加上 8 小时以调整时区
  ...
  if (str == "firstDay") return formattedFirstDay;
  if (str == "lastDay") return formattedLastDay;
